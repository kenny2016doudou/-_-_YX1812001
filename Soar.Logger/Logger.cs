...
namespace Soar.Framework
{
    public class Logger : ILogger
    {
        ...
        public void openFile()
        {
            ...
            Process.Start(logDir+logEntity.CurrentFileName);
        }
        /// <summary>
        /// 启动日志组件
        /// </summary>
        ...
        public bool Run(LogLevel logLevel, string logDirectory, int maxFileSize)
        {
            ...
            {
                logEntity.CurrentFileName = DateTime.Now.ToString("yyyy-MM-dd") + ".txt";
                    ...
                    lock (syncRoot)
                    {
                        ...
                        foreach (LogItem item in logEntity.ListLogItem)
                        {
                            ...
                            if (fileSize > logEntity.MaxFileSize)
                            {
                                ...
                                logEntity.CurrentFileName = logEntity.CurrentFileName.Substring(0, 10) + "_" + logEntity.FileSymbol + ".txt";
                                CreateFile(logDir + logEntity.CurrentFileName);
                                write.Flush();
                                write.Close();
                                write.Dispose();
        ...
        private void MonitorLog()
        {
            ...
            if (dtNow != dtOld)
            {
                lock (syncRoot)
                {
                    logEntity.CurrentFileName = dtNow + ".txt";
                    logEntity.FileSymbol = 0;
        ...
        public void LoadLogger(IList<string> listFileName)
        {
            ...
            logEntity.CurrentFileName = tempJsonList[maxIndex].Key.ToString("yyyy-MM-dd") + (tempJsonList[maxIndex].Value == 0 ? "" : "_" + tempJsonList[maxIndex].Value) + ".txt";
