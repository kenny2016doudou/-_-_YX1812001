using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using System.Windows.Forms;
using System.IO.Ports;
using System.Threading;


namespace ZZ.Serial
{
    public class YXDZ
    {
         private static YXDZ uniqueInstance;

         private YXDZ() { }

         public static YXDZ getInstance()
        {
            if (uniqueInstance == null) uniqueInstance = new YXDZ();

            return uniqueInstance;
        }

        public void OpenPort(SerialPort sp, string PortName, Parity parity, int BaudRate, int DataBits, StopBits stopBits)
        {
            try
            {
                if (sp != null)
                {
                    sp.PortName = PortName;
                    sp.Parity = parity;
                    sp.BaudRate = BaudRate;
                    sp.DataBits = DataBits;
                    sp.StopBits = stopBits;
                    sp.Open();
                }
            }
            catch
            {
 
            }
        }
        /// <summary>
        /// 写数据
        /// </summary>
        /// <param name="sp"></param>
        /// <returns></returns>
        public static bool WriteData(System.IO.Ports.SerialPort sp, byte[] ZL, int xh ,bool wOk)
        {

            if (!sp.IsOpen)
            {
                sp.Open();
            }
            sp.DiscardInBuffer();
            sp.DiscardOutBuffer();
            //byte[] b = new byte[4];

            sp.DiscardInBuffer();
            System.Threading.Thread.Sleep(20);
            sp.Write(ZL, 0, ZL.Length);
            System.Threading.Thread.Sleep(100);

            string str = "";

            if (sp.IsOpen && sp.ReadBufferSize > 0)
            {

                sp.DiscardOutBuffer();
                //System.Threading.Thread.Sleep(100);
                //str = sp.ReadExisting();
            }
            byte[] buffer = new byte[0x39];
            if (sp.IsOpen && sp.BytesToRead != 0)
            {
                sp.Read(buffer, 0, 57);
                str = sp.ReadExisting();
            }
#pragma warning disable CS0219 // 变量“redstr”已被赋值，但从未使用过它的值
            string redstr = "";
#pragma warning restore CS0219 // 变量“redstr”已被赋值，但从未使用过它的值

            if (buffer[xh*5] == 0)
            {
                wOk = true;
            }

            return wOk;
        }
        /// <summary>
        /// 单个读数据
        /// </summary>
        /// <param name="sp"></param>
        /// <returns></returns>
        public static string ReadData(System.IO.Ports.SerialPort sp, byte[] ZL, int xh)
        {
            if (!sp.IsOpen)
            {
                sp.Open();
            }
            //sp.DiscardInBuffer();
            //sp.DiscardOutBuffer();
            byte[] b = new byte[4]; byte[] b1 = new byte[4]; byte[] b2 = new byte[4];

            sp.DiscardInBuffer();
            System.Threading.Thread.Sleep(200);
            sp.Write(ZL, 0, ZL.Length);
            System.Threading.Thread.Sleep(500);

            string str = "";

            if (sp.IsOpen && sp.ReadBufferSize > 0)
            {

                sp.DiscardOutBuffer();
                //
                //str = sp.ReadExisting();
            }
            byte[] buffer = new byte[0x400];
            System.Threading.Thread.Sleep(500);
            if (sp.IsOpen && sp.BytesToRead != 0)
            {
                sp.Read(buffer, 0, 57);
                str = sp.ReadExisting();
            }
#pragma warning disable CS0219 // 变量“redstr”已被赋值，但从未使用过它的值
            string redstr = "";
#pragma warning restore CS0219 // 变量“redstr”已被赋值，但从未使用过它的值

            //if (buffer[xh*15] == 4)
            //{
            //    redstr += Convert.ToString((buffer[16]), 16) + Convert.ToString((buffer[17]), 16) + Convert.ToString((buffer[18]), 16) + Convert.ToString((buffer[19]), 16);
            //}
            b[3] = Convert.ToByte(Convert.ToString((buffer[xh * 5 + 2])));
            b[2] = Convert.ToByte(Convert.ToString((buffer[xh * 5 + 3])));
            b[1] = Convert.ToByte(Convert.ToString((buffer[xh * 5 + 4])));
            b[0] = Convert.ToByte(Convert.ToString((buffer[xh * 5 + 5])));
       
            
          
            Single DZ = BitConverter.ToSingle(b, 0)/10000000;
            //Single DZ = BitConverter.ToSingle(b, 0);
            string STR = Convert.ToString(DZ);

            return STR;


        }
        /// <summary>
        /// 批量读数据
        /// </summary>
        /// <param name="sp"></param>
        /// <returns></returns>
        public static byte[] ReadAllData(System.IO.Ports.SerialPort sp, byte[] ZL, int xh)
        {
            if (!sp.IsOpen)
            {
                sp.Open();
            }
            //sp.DiscardInBuffer();
            sp.DiscardOutBuffer();
            byte[] DZZ = new byte[57];
            byte[] b = new byte[4]; ;

            sp.DiscardInBuffer();
            System.Threading.Thread.Sleep(20);
            sp.Write(ZL, 0, ZL.Length);
            System.Threading.Thread.Sleep(1000);

            //string str = "";

            //if (sp.IsOpen && sp.ReadBufferSize > 0)
            //{

            //    sp.DiscardOutBuffer();
            //    //
            //    //str = sp.ReadExisting();
            //}
            byte[] buffer = new byte[0x39];
            //System.Threading.Thread.Sleep(200);
            //sp.DiscardInBuffer();
            sp.DiscardOutBuffer();
            if (sp.IsOpen && sp.BytesToRead != 0)
            {
                sp.Read(buffer, 0, 57);
                //str = sp.ReadExisting();
            }

            DZZ = buffer;
#pragma warning disable CS0219 // 变量“redstr”已被赋值，但从未使用过它的值
            string redstr = "";
#pragma warning restore CS0219 // 变量“redstr”已被赋值，但从未使用过它的值

            //if (buffer[xh*15] == 4)
            //{
            //    redstr += Convert.ToString((buffer[16]), 16) + Convert.ToString((buffer[17]), 16) + Convert.ToString((buffer[18]), 16) + Convert.ToString((buffer[19]), 16);
            //}
            //b[3] = Convert.ToByte(Convert.ToString((buffer[xh * 5 + 1])));
            //b[2] = Convert.ToByte(Convert.ToString((buffer[xh * 5 + 2])));
            //b[1] = Convert.ToByte(Convert.ToString((buffer[xh * 5 + 3])));
            //b[0] = Convert.ToByte(Convert.ToString((buffer[xh * 5 + 4])));



            //Single DZ = BitConverter.ToSingle(b, 0) / 1000000;

            //string STR = Convert.ToString(DZ);

            return DZZ;


        }
        /// <summary>
        /// 通讯
        /// </summary>
        /// <param name="commandText">发送的命令，位数必须为偶数</param>
        public string CommandTextStr(string commandText,System.IO.Ports.SerialPort sp)
        {
            ModBus mb = new ModBus();
            if (commandText.Length % 2 != 0)
            {
                MessageBox.Show("通讯命令格式错误！");
                return "00";
            }
            else
            {
                string m_CommandText = "";
                string temp = "";
                byte[] buf1 = new byte[7];
                byte[] buf = new byte[5];
                //字符串转为数组
                for (int i = 0; i < 5; i++)
                {
                    buf[i] = byte.Parse(commandText.Substring(i * 2, 2), System.Globalization.NumberStyles.HexNumber);
                    buf1[i] = buf[i];
                }
                temp = mb.CalculateCrc16(buf).ToString("X");//CRC校验，并输出大写十六进制
                for (int i = temp.Length; i < 4; i++)
                {
                    temp = "0" + temp;
                }
                //数据补0，确保为00格式，而不是0格式。
                for (int i = 0; i < commandText.Length; i++)
                {
                    if (commandText[i] < 16)
                    {
                        m_CommandText += "0" + commandText[i].ToString();
                    }
                    else
                    {
                        m_CommandText += commandText[i].ToString();
                    }
                }
                buf1[5] = byte.Parse(temp.Substring(0, 2), System.Globalization.NumberStyles.HexNumber);
                buf1[6] = byte.Parse(temp.Substring(2, 2), System.Globalization.NumberStyles.HexNumber);
                m_CommandText += temp;
                sp.Write(buf1, 0, 7);

                byte[] buffer = new byte[8];
                System.Threading.Thread.Sleep(100);
                sp.Read(buffer, 0, 8);
                sp.DiscardInBuffer();
                sp.DiscardOutBuffer();
                byte[] intBuffer = new byte[8];
                //高低字节转换
                for (int i = 0; i < 8; i++)
                {
                    intBuffer[i] = buffer[7-i];
                }
                string redstr = "";
                if (intBuffer != null)
                {
                    if (intBuffer.Length >= 4)
                    {
                        float a=float.Parse(((intBuffer[1] * Math.Pow(2,8) + intBuffer[0])+1).ToString());
                        float b=(float)10;
                        redstr = (a/b).ToString();
                    }
                }

                return redstr;
            }
        }
        private byte[] CRC16(byte[] data)
        {
            byte CRC16Hi;
            byte CRC16Lo;
            CRC16Hi = 0xFF;
            CRC16Lo = 0xFF;
            long iIndex;
            for (int i = 0; i < data.Length; i++)
            {
                iIndex = CRC16Hi ^ data[i];
                CRC16Hi = (byte)(CRC16Lo ^ _crcHi[iIndex]); //低位处理
                CRC16Lo = _crcLo[iIndex]; //高位处理
            }

            byte[] ReturnData = new byte[2];
            ReturnData[0] = CRC16Hi; //CRC高位
            ReturnData[1] = CRC16Lo; //CRC低位

            return ReturnData;
        }


        private readonly byte[] _crcLo = new byte[]//CRC低位字节值表
        { 
            
        };

        private readonly byte[] _crcHi = new byte[] //CRC高位字节值表
        { 
            
        };

        /// <summary>
        /// 启动
        /// </summary>
        /// <param name="sp"></param>
        public static void QiDong(System.IO.Ports.SerialPort sp)
        {
            if (!sp.IsOpen)
            {
                sp.Open();
            }
            System.Threading.Thread.Sleep(100);

            byte[] b = new byte[2];
            b[0] = Convert.ToByte(35);
            b[1] = Convert.ToByte(71);

            sp.Write(b, 0, b.Length);
            System.Threading.Thread.Sleep(20);
            sp.DiscardInBuffer();
            System.Threading.Thread.Sleep(20);
            sp.DiscardOutBuffer();
            System.Threading.Thread.Sleep(20);
        }

        /// <summary>
        /// 停止
        /// </summary>
        /// <param name="sp"></param>
        public static void TingZhi(System.IO.Ports.SerialPort sp)
        {
            try
            {
                if (!sp.IsOpen)
                {
                    sp.Open();
                }
                byte[] b = new byte[2];
                b[0] = Convert.ToByte(35);
                b[1] = Convert.ToByte(85);
                for (int i = 0; i < 5; i++)
                {

                    sp.DiscardInBuffer();
                    System.Threading.Thread.Sleep(20);
                    sp.DiscardOutBuffer();
                    System.Threading.Thread.Sleep(20);
                    sp.Write(b, 0, b.Length);
                    System.Threading.Thread.Sleep(20);
                    if (sp.ReadExisting() != "")
                    {
                        break;
                    }

                }
            }
            catch { }
        }
        public static void OpenPort(SerialPort sp, string PortName)
        {
            try
            {
                if (!sp.IsOpen)
                {
                    //通讯控件设置
                    sp.PortName = PortName;
                    sp.StopBits = StopBits.One;
                    sp.Parity = Parity.None;
                    sp.BaudRate = 115200;
                    sp.DataBits = 8;

                    sp.Open();
                }
            }
            catch  { }
        }
    }
}
