...
namespace TestSystem.Command.YB.TianChen
{
    public class TianChengYB
    {
        ...
        public float GetSend(string command)
        {
            ...
            {
                if (getStr != "0")
                    getStr = getStr.Split('=')[1].Substring(0, 6);
            }
            catch { }
            float reValue = 0;
            try
        ...
        public float GetSetValue(string command)
        {
            string getStr = SendStr(command);
            try
            {
                if (getStr != "0")
                    getStr = getStr.Split('!')[1].Substring(0, 7);
            }
            catch { }
            float reValue = 0;
            try
        ...
        {
            sp.Write(SendStr("%" + addr + "10+01111"));
            ...
            sp.Write(SendStr("%" + addr + "10+00000"));
            System.Threading.Thread.Sleep(100);
        }
